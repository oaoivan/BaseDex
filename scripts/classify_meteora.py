#!/usr/bin/env python3
"""Classify Meteora pools by program owner via Solana RPC."""
...
OUT_FILE = os.path.join(ROOT_DIR, "classified_pools_meteora.json")
LOG_FILE = os.path.join(ROOT_DIR, "term.txt")

PROGRAM_OWNERS: Dict[str, Tuple[str, str]] = {
    "LBUZKhRxPF3XUpBCjp4YzTKgLccjZhTSDM9YuVaPwxo": (
        "dlmm",
        "Meteora DLMM (concentrated bins)",
    ),
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Classify Meteora pools by Solana program owner.",
...
def iter_meteora_addresses(pools: dict):
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    entries = list(iter_meteora_addresses(pools))
    total = len(entries)
    _log(f"total meteora candidates: {total}; rpc={args.rpc}")
